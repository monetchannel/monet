{extends file="watch_video_header.tpl"}
	...
	<div class="col-md-12" >
		...
		<div id="videoPanel" class="panel panel-default">
			<div class="panel-heading">
				<strong><em>{$content.c_title}!</em></strong>
											...
											<ul id="accordionOne" class="panel-collapse collapse">
												<li class="sub-nav">
													<a href="javascript:void(0)" id="fbshare" class="fbshare">
														<img class="img-responsive" src="images/fbshare.png"/>
													</a>
												</li>
												<li class="sub-nav">
													<a href="#">
														<img class="img-responsive" src="images/twitter-share-button.png"/>
													</a>
												</li>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<form id="ratingfrm" role="form">
					...
					<input type="hidden" value="{$cf_ch_id}" name="cf_ch_id">
					<input type="hidden" value="{$cf_id}" name="cf_id">
					<input type="hidden" value="{$content.instanceID}" name="videoID">
					...
					<div>
						...
						<textarea name="comment" id="comment" class="form-control text-area-field custom-text-area" 
                                                          placeholder="Please enter your comments" ></textarea>
                                            
                                            
						{if $questions|@count > 0}
						<ul type="number" style="list-style-type: none;padding: 0px;margin: 0px;" class="list-style-none">
						{$temp=1}
							{foreach $questions as $q=>$op}
                                                            <li class="form-group" style="list-style-type: unset;">
								<div class="video-label">{$op.q_question}</div>
								<div class="styled-select">
                                                                        <!--
									<select name="ans[{$op.q_id}]">
										<option value="">-Select-</option>
										{foreach $op.option as $opk=>$opv}
										<option value="{$opk}">{$opv}</option>
										{/foreach}
									</select>
                                                                        -->
                                                                        <select id="answer{$temp}" name="ans[{$op.q_id}]">
                                                                                <option value="">-Select-</option>
						...
						<div  class="col-md-12" style="margin-top:-6px; padding-bottom: 4%;text-align: center;">
							<button id="graphSubmit" type="submit" class="submit-button">Submit</button>							
						</div>	
						<div class="col-md-8 col-md-offset-3" style="text-align: center;" align="center">
                                                    
                                                    <ul id="accordionModal" class="nav-tabs social-media" style=" border-bottom: medium none;">
							<li class="sub-nav">
...
// 1. This code loads the IFrame Player API code asynchronously.
	...
	function onYouTubeIframeAPIReady() {
		...
		player = new YT.Player('player', {
			...
			playerVars:{literal} {'rel':0 }{/literal},
...
//navigator.getUserMedia = navigator.getUserMedia || navigator.webkitGetUserMedia || navigator.mozGetUserMedia || navigator.msGetUserMedia;
 ...
 window.getUserMedia = function (options, successCallback, errorCallback) {
  ...
  if (options !== undefined) {
	...
	{literal}video.addEventListener('playing',startDraw);{/literal}
...
$(function() {
	...
	*/
	...
	$('#browse-more').on('click',function(){
        ...
        $('#ratingfrm').on('submit', function(e){
            var answer_one='', answer_two='', comment_txt='';
            comment_txt = $(this).find('#comment').val();
            answer_one = $(this).find('#answer1').val(); 
            var frm = $( this ).serialize()
            
            var answer_two_question_exists = $(this).find('#answer2').length;
            if(answer_two_question_exists>0){
                answer_two = $(this).find('#answer2').val();
            }
            
            if(comment_txt=="" || comment_txt=='undefined'){
                 alert("Comment box cannot be left blank.");
                 return false
            } else if (answer_one=='' || answer_one=='undefined'){
                 alert("Please select answer for first question.");
                 return false
            } else if(answer_two=='' || answer_two=='undefined'){
                 alert("Please select answer for second question also.");
                 return false; 
            } else {
                 $.ajax({
                    {literal}
	...
	var loca = window.location.href, title = $('title').text(), vlink = "https://www.youtube.com/embed/"+ "{$video_id}";
