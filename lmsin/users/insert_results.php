	    ...
	    $expressions = $expressions[0];
 		if($expressions!=null)								//null in Faces
 		{
	 		...
	 		$facial = $expressions['FacialExpression']; 
                        ...
                        // Code for Engagement, Headstillness and eye stillness 
                        $eye=0.25;
 			$head=0;
 			$valid_model=1;	
 			$x=0;
 			...
 			if($eye>1) $eye=1;
 				...
 				//$head=$head + $expressions['HeadOrientation'][$i]*$expressions['HeadOrientation'][$i];
                            $head = $head + pow($expressions['HeadOrientation'][$i], 2);
                        }
 			...
 			$y=$y/4;
 				...
 				$engagement = ($engagement>1) ? 1 : $engagement;
				$engagement = ($engagement<-1) ? -1 : $engagement;
 			}
 			else{
 				for($i = 0; $i<4; $i++){
 			...
 			if($facial != null)								//null in FacialExpression
 			{
			 	...
			 	{
			 		$count++;
			 		$SQL1= "INSERT into analysis_detail (ad_ar_id,ad_time,ad_neutral,ad_happy,ad_sad,ad_angry,ad_suprised,ad_scared,ad_disgusted,ad_dominant_emotion, ad_valence, ad_gender,ad_age,ad_glasses,ad_moustache, ad_engagement) VALUES ($row[ar_id], '$time', $facial[Neutral], $facial[Happy], $facial[Sad],$facial[Angry],$facial[Surprised],$facial[Scared],$facial[Disgusted],'$index',$facial[Happy]-$maxneg,'$expressions[Gender]','$expressions[Age]','$expressions[Glasses]','$expressions[Moustache]', '$engagement')";
			 		$ids = ei($SQL1);
			 	...
			 	if($count==0)
			 	{
			 		...
			 		$ids = ei($SQL1);
			 		$SQL1= "INSERT into analysis_detail (ad_ar_id,ad_time,ad_neutral,ad_happy,ad_sad,ad_angry,ad_suprised,ad_scared,ad_disgusted,ad_dominant_emotion,ad_valence,ad_gender,ad_age,ad_glasses,ad_moustache,ad_engagement) VALUES($ids,'$time',$facial[Neutral],$facial[Happy],$facial[Sad],$facial[Angry],$facial[Surprised],$facial[Scared],$facial[Disgusted],'$index',$facial[Happy]-$maxneg,'$expressions[Gender]','$expressions[Age]','$expressions[Glasses]','$expressions[Moustache]', '$engagement')";
					$ids = ei($SQL1);
					$SQL1 = "INSERT into analysis_results_average (ara_c_id,ara_time,ara_valence) VALUES($c_id,$totalseconds,$facial[Happy]-$maxneg)";
			 		mysql_query($SQL1) or die(mysql_errno());
			 		$valence=$facial[Happy]-$maxneg;
