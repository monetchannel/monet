...
//.........................................
$func_ary=array("show_login","chk_login","dashboard","save_ragistration","logout","analysis_results","settings","analysis_graph","import_file","import_do","update_settings","show_analysis_graph","analysis_csv","update_ad_valence","company_profile_edit","company_profile_update","save_invites_request","chk_login_brand");
#############################################################
if (($_COOKIE["CompanyId"]=="" AND $_REQUEST[act]!="show_login" AND $_REQUEST[act]!="chk_login" && $_REQUEST[act]!="save_ragistration" && $_REQUEST[act]!="save_invites_request" && $_REQUEST[act]!="chk_login_brand"))
{ #not logged in
   show_login();
   return;
}
...
function show_login($msg="" ,$comp_url='')
{
	...
	if($R[company_url])
	{
		...
		if(get_upload_info($company[company_id],"company_logo",0,$company_logo)>0)
			$smarty->assign("company_logo", $company_logo[up_thumb_view_path]);
		else
			$smarty->assign("company_logo",'');		
	}
	$smarty->assign(array("msg"=>$msg,"none"=>$none,"SERVER_COMPANY_PATH"=>$Server_company_Path,
						 ...
						 "SERVER_PATH"=>$Server_View_Path,"company_url"=>$R[company_url],"company_name"=>$company[company_name]));
	if($R[company_url])
...
function logout()
{
	...
	if($R[company_url])
		$path=$Server_company_Path.$com[company_url];
	else	
		$path=$Server_company_Path;
	header("Location:$path");
  /// show_login("<p align=center> You have been successfully logged out. </p>",$com[company_url]);
}
###########################################################
...
function chk_login_brand()
{
	...
	else
	{
		...
		print "<script>parent.window.return_brand_msg('',true,'$user[user_accept_toc]','".$R[uf_id]."');</script>";
	}	
}
