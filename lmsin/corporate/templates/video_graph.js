...
var overall_micro_exhilarated=[],overall_micro_delighted=[],overall_micro_blissful=[],overall_micro_serene=[],overall_micro_depressed=[],overall_micro_despairing=[],overall_micro_disgusted=[],overall_micro_terrified=[],overall_micro_furious=[],overall_micro_interested=[];
var overall_micro_happy=[],overall_micro_pleased=[],overall_micro_content=[],overall_micro_relaxed=[],overall_micro_bored=[],overall_micro_sad=[],overall_micro_afraid=[],overall_micro_angry=[];
...
var time_covered=0;
function everyTick(totalTime){
	if(time_covered<=totalTime){
		while(time_covered<=totalTime){
			var micro_eng=microexpressions_overall[time_covered][1];
		    var micro_val=microexpressions_overall[time_covered][2];
		    var dist=99,min_dist=99,micro_value;
		    for(var k=0;k<18;++k){
		    	if(micro_emotions[k]['x']*micro_val>=0&&micro_emotions[k]['y']*micro_eng>=0){       // Check if they are in the same quadrant
		    		dist=Math.sqrt(Math.pow((micro_emotions[k]['x']-micro_val), 2)+Math.pow((micro_emotions[k]['y']-micro_eng), 2));
		    		if(dist<min_dist){
		    			min_dist=dist;
		    			micro_value=micro_emotions[k]['values'];
		    ...
		    if(micro_value=="exhilarated") overall_micro_exhilarated.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="delighted") overall_micro_delighted.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="blissful") overall_micro_blissful.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="serene") overall_micro_serene.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="depressed") overall_micro_depressed.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="despairing") overall_micro_despairing.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="disgusted") overall_micro_disgusted.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="terrified") overall_micro_terrified.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="furious") overall_micro_furious.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="interested") overall_micro_interested.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="happy") overall_micro_happy.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="pleased") overall_micro_pleased.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="content") overall_micro_content.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="relaxed") overall_micro_relaxed.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="bored") overall_micro_bored.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="sad") overall_micro_sad.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="afraid") overall_micro_afraid.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
		    else if(micro_value=="angry") overall_micro_angry.push({x: micro_val, y: micro_eng, size: 1-min_dist, shape:"circle"});
			time_covered++;
		}
	}
	else time_covered++;
	if(time_covered<=totalTime+2){
	    var micro_data=[
		    {
		    	key: 'exhilarated',
	            values: overall_micro_exhilarated
		    },
		    {
		    	key: 'delighted',
	            values: overall_micro_delighted
		    },
		    {
		    	key: 'blissful',
	            values: overall_micro_blissful
		    },
		    {
		    	key: 'serene',
	            values: overall_micro_serene
		    },
		    {
		    	key: 'depressed',
	            values: overall_micro_depressed
		    },
		    {
		    	key: 'despairing',
	            values: overall_micro_despairing
		    },
		    {
		    	key: 'disgusted',
	            values: overall_micro_disgusted
		    },
		    {
		    	key: 'terrified',
	            values: overall_micro_terrified
		    },
		    {
		    	key: 'furious',
	            values: overall_micro_furious
		    },
		    {
		    	key: 'interested',
	            values: overall_micro_interested
		    },
		    {
		    	key: 'happy',
	            values: overall_micro_happy
		    },
		    {
		    	key: 'pleased',
	            values: overall_micro_pleased
		    },
		    {
		    	key: 'content',
	            values: overall_micro_content
		    },
		    {
		    	key: 'relaxed',
	            values: overall_micro_relaxed
		    },
		    {
		    	key: 'bored',
	            values: overall_micro_bored
		    },
		    {
		    	key: 'sad',
	            values: overall_micro_sad
		    },
		    {
		    	key: 'afraid',
	            values: overall_micro_afraid
		    },
		    {
		    	key: 'angry',
	            values: overall_micro_angry
		    },
		    {
	    		key: 'corners',
	    		values: nothing
...
function show_smiley(i, time,idx)
{
		...
		if(i==dom_emotion[3]){
			...
			if(!done) document.getElementById('timeline').innerHTML+="<div class=\"timeline-item\" id=\"at-"+time+"\" style=\"left:"+(timeline_width*time/video_duration-7).toString()+"px;\"><span class=\"dot\"></span><div class=\"img-popup\"><img src=\"/Monet/uploads/"+json[idx][dom_emotion[3]][8]+".jpg\" width=\"120\" height=\"90\" /></div></div>";
			p++;
		...
		if(i==dom_emotion[5]){
			...
			if(!done) document.getElementById('timeline').innerHTML+="<div class=\"timeline-item\" id=\"at-"+time+"\" style=\"left:"+(timeline_width*time/video_duration-7).toString()+"px;\"><span class=\"dot\"></span><div class=\"img-popup\"><img src=\"/Monet/uploads/"+json[idx][dom_emotion[5]][8]+".jpg\" width=\"120\" height=\"90\" /></div></div>";
			p++;
