...
//require '../includes/google-api-php-client/src/Google/Client.php';
//require '../includes/google-api-php-client/src/Service/Plus.php';
init();
...
function chk_login()
{
	...
	$R[user_password]=md5($R[user_password]);
        if (!get_row_con_info("users","where user_email='$R[user_email]' limit 0,1","",$user)) {//no email
            $user_activated = -1;
        }else {
            $user_activated_SQL = "SELECT user_activated FROM users WHERE user_email='$R[user_email]'";
            eq($user_activated_SQL, $result);
            $user_activated = mfa($result)["user_activated"];
        }
	if(!get_row_con_info("users","where user_email='$R[user_email]' AND user_password='$R[user_password]' AND user_activated>0 limit 0,1","",$user))
	{
                //reset session vars to prevent accidental logins
                $_SESSION["google_token"] = '';
                $_SESSION["fb_token"] = '';
                if ($user_activated == 0) {//account not activated
                    $ary[0]="Your account has not been activated. Check your email for the activation link."; //We used jquery ajax so use print
                }else if ($user_activated == 2){//fb
                    $ary[0]="Your account is registered for Facebook login. Please sign in with Facebook to continue."; 
                }else if ($user_activated == 3) {//google
                    $ary[0]="Your account is registered for Google login. Please sign in with Google to continue."; 
                }else {
                    $ary[0]="The email address and password you entered does not match our records. Please try again."; 
                }
                $ary[1]=1;
...
function create_new_user(){//replaces save_invites_request and removes need for admin approval of new users
	...
	global $Server_View_Path;
        ...
        if(!filter_var($R[user_email], FILTER_VALIDATE_EMAIL)){
            ...
            $user_dob = $R["user_dob"];
            $user_activation_num = md5($user_email);
        
            $add_user_SQL = "INSERT INTO users (user_fname, user_lname, user_email, user_password, user_gender, user_dob, user_max_invites, user_accept_toc, user_zipcode, user_country, user_activated,user_activation_num)"
                        ."VALUES ('$user_fname', '$user_lname', '$user_email', '$user_password','$user_gender','$user_dob','10', '0', '$user_zip', '$user_country','$activated','$user_activation_num')";
            eq($add_user_SQL, $result);
            
            $email_link = "$Server_View_Path/user/confirmation.php?act=display&number=".md5($user_email);//confirmation number is md5 hash of email
            $email_message = "<p>Click <a href='$email_link'>here</a> to activate your account.</p>";
            send_mail_new("$user_fname $user_lname", $user_email, "", "", "Monet Email Confirmation", $email_message, "");
            print "Account creation successfull, please check your email to activate account.";
        }
}
...
function google_login() {
    ...
    if (isset($_REQUEST["code"])) {
        if (isset($_SESSION['google_token']) && $_SESSION['google_token'] != ''){//user already signed in this session
            $google_client->setAccessToken($_SESSION['google_token']);
        }else {//first sign in this session
            $google_client->authenticate($_REQUEST["code"]);
            $jsonTokens = $google_client->getAccessToken();
            $_SESSION['google_token'] = $jsonTokens;
        }
        $me = $plus->people->get("me");
        $verified_email = $oauth2->userinfo->get()["verified_email"];
        if (!$verified_email){//user has not verified their email
            print "Please verify email with Google to continue.";
        }else {
            $user_email = $oauth2->userinfo->get()["email"];//get email
            $user_pass =  $me["id"].md5($google_secret);//generate password
            if (!get_row_count("users", "WHERE user_email='$user_email' limit 0,1")) {//email not already existing
                $_REQUEST["user_email"] = $user_email;
                $_REQUEST["user_password"] = $user_pass;
                $_REQUEST["user_fname"] = $oauth2->userinfo->get()["givenName"];
                $_REQUEST["user_lname"] = $oauth2->userinfo->get()["familyName"];
                $_REQUEST["user_gender"] = $me["gender"];
                $_REQUEST["user_dob"] = $me["birthday"];
                $_REQUEST["user_activated"] = '3';
                create_new_user();
                chk_login();
            }else{
                $_REQUEST["user_email"] = $user_email;
