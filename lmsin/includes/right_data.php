...
function get_right_column_data($type="",$cat_id="",$st_pos="",$nrpp="")
{
	...
	//-------------------------------------------------------------------------------------------------------------
	$people=array();
	$SQL="select * from users where user_id IN (Select uf_fuser_id  from user_friends where uf_user_id  IN (Select uf_fuser_id  from user_friends where uf_user_id = '$_COOKIE[UserId]' UNION Select uf_user_id  from user_friends where uf_fuser_id = '$_COOKIE[UserId]' )
UNION
Select uf_user_id  from user_friends where uf_fuser_id IN (Select uf_fuser_id  from user_friends where uf_user_id = '$_COOKIE[UserId]' UNION Select uf_user_id  from user_friends where uf_fuser_id = '$_COOKIE[UserId]')  ) AND user_id<>'$_COOKIE[UserId]'

 AND user_id NOT IN 
(Select uf_fuser_id  from user_friends where uf_user_id = '$_COOKIE[UserId]'  UNION Select uf_user_id  from user_friends where uf_fuser_id = '$_COOKIE[UserId]' ) order by rand() LIMIT 0,5";
	$tot_rows=eq($SQL,$rs2);
	while($data2=mfa($rs2))
	{
