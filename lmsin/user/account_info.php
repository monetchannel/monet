...
function save_profile_changes(){
    ...
    } if ($_REQUEST["user_lname"] != $user_current_vals["lname"]){
        update_profile("user_lname", $_REQUEST["user_lname"]);
        
    } if ($_REQUEST["user_dob"] != $user_current_vals["dob"]){
        update_profile("user_dob", $_REQUEST["user_dob"]);
    } if ($_REQUEST["user_email"] != $user_current_vals["email"] && ($_COOKIE['uses_social_login'] != "1")){
        update_profile("user_email", $_REQUEST["user_email"] );
    } if ($_REQUEST["user_zip"] != $user_current_vals["zipcode"]){
        update_profile("user_zipcode", $_REQUEST["user_zipcode"]);
    } if (($_REQUEST["user_country"] != $user_current_vals["country"]) && ($_REQUEST["user_country"] == -1)){
