...
init();
//.........................................
sajax_export("video_add","video_view","video_edit","video_del","view_feedback");
...
function video_view($callback,$msg="",$orderby_p="",$order_p="",$st_pos_p=0,$nrpp_p=0,$ser_key="")
{
	...
	$SQL="SELECT * ,(SELECT count(*) FROM content_feedback WHERE cf_c_id = c_id AND cf_rating>'0' and cf_ep_id>'0') AS num_feedback FROM content WHERE 1 $cond ORDER BY $orderby $order $con_limit";
	$tot_rows=eq($SQL,$rs);
...
function video_add($callback)
{
	...
	get_new_option_list("company","company_id","company_name",$company_id,$company_list," order by company_name");
	$smarty->assign($R);
...
function video_edit($callback,$c_id)
{
	...
	get_new_option_multiple_list("category","cat_id","cat_name",$cat_ids,$option," order by cat_name");
	get_new_option_list("company","company_id","company_name",$video_info[c_company_id],$company_list," order by company_name");
...
function video_update()
{
	...
	$SQL="UPDATE content SET $url `c_date` =  '$c_date',
	                              `c_av_challenge` = '$R[c_av_challenge]',
								  `c_company_id`='$R[c_company_id]' WHERE c_id = '$R[c_id]'";
	eq($SQL,$rs);
