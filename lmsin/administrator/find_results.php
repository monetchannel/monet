<?php
	...
	$SQL = "SELECT * FROM `emotion_average` ORDER BY  `emotion_average`.`ea_id` ASC ";
	eq($SQL,$res17);
	$result=array();
	...
	$SQL = "SELECT * FROM `user_profile` WHERE up_user_id IN (SELECT cf_user_id FROM content_feedback WHERE cf_id =".$cf_id.") ORDER BY  `user_profile`.`up_emotion_id` ASC ";
	eq($SQL,$res19);
	$result=array();
	while($data = mfa($res19))
	{
		array_push($result,array($data[up_emotion_id],$data[up_avg_eng],$data[up_avg_val],$data[up_avg_neutral],$data[up_avg_happy],$data[up_avg_sad],$data[up_avg_angry],$data[up_avg_suprised],$data[up_avg_scared],$data[up_avg_disgusted]));
	}
	$res20=json_encode($result);

	//echo $count;
	echo "[".$res1.",".$res3.",".$res6.",".$res12.",".$res8.",".$res14.",".$res18.",".$res20."]";
	//echo "[".$res1.",".$res3."]";
