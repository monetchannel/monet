...
//.........................................
$func_ary=array("report_export","feedback_view","watch_video","watch_slides");
###################### Ok #####################################
if($_SESSION["Admin"]=="")
{ #not logged in
...
function feedback_index($msg='')
{
	...
	$smarty = new Smarty;
	$smarty->assign(array("msg"=>$msg,"js"=>$js,"SERVER_PATH"=>$Server_View_Path,"feedback_tab"=>"selected","invit_num"=>$invit_num,"company_invite_num"=>$company_invite_num));
  ...
  if($date_from!="") 
   ...
   if($tot_rows>0)
   {
      while($data=mfa($rs))
      {
			...
			{				
				$video_type="flv";
				$data[video]= "<span id=button><a href=\"javascript:void(1)\" onclick=\"setLink('".$url."','".$video_type."')\"><img src='images/watch.gif' border='0'/></a></span>";
			...
			if(sizeof($pics)){
				$data[slides]="<span id=slide_button><button href=\"javascript:void(1)\" onclick=\"setLinkSlide('".$ad_ar_id."')\">Slide</button></span>";
			} 
   ...
   else
   {
	...
	if($R[type]=='export')
	{
		export_user_feedback($export_data,$video_file);
	}
	
	get_new_option_list("content_feedback LEFT JOIN content ON cf_c_id=c_id","c_id","c_title",$v_title,$title,"1","order by c_title asc","0","DISTINCT c_id,c_title");
	get_new_option_list_full_name("content_feedback LEFT JOIN users ON cf_user_id=user_id","user_id","user_fname","user_lname",$v_user,$se_user,"1","order by user_fname asc","0","DISTINCT user_id,user_fname,user_lname");
	get_new_option_multiple_list("content_feedback left join emotional_profile on cf_ep_id=ep_id","ep_id","ep_name", $se_emo,$ep,"","DISTINCT ep_id,ep_name");
...
function watch_video()
{
	...
	$smarty->assign(array("video_type"=>$_REQUEST[video_type]));
...
function export_user_feedback($export_data,$video_file)
{
		global $Upload_Path;
		global $Server_Upload_Path;
		...
		{
			foreach($video_file as $k=>$v)
			{
				$file=$Server_Upload_Path."video_files/".$v;
				$zip->addFile(file_get_contents($file),$v);
			}
		}
