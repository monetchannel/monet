  ...
  if($date_from!="") 
   ...
   //Exprot data
	if($R[type]=='export')
	{
		...
		print "\"User Name\",\"Video Name\",\"Comment\",\"Date\",\"Monet Profile\",\"Rating\",\"Video File Name\"\r\n";
	}
   ...
   if($tot_rows>0)
   {
      while($data=mfa($rs))
      {
		...
		if($R[type]=='export' && file_exists($file))
		{
			...
			print "\"$data[user]\",\"$data[c_title]\",\"$data[cf_comment]\",\"$data[cf_date]\",\"$data[ep_name]\",\"$data[cf_rating]\",\"$file_name\"\r\n";
		}
		//-------------------------------End
