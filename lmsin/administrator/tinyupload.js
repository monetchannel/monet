function tinyupload(field_name, url, type, win){
	...
	*/
	...
	CONFIG
	*/
	//var pathToPhp = 'http://portalfa.securesites.net/tinyupload.php';
	//var pathToPhp = 'http://portalforce.com/tinyupload.php';
	var pathToPhp = 'http://192.168.5.22/siri/new/tinyupload.php';
	/*
	...
	*/
	...
	el1.style.cssText = 'background:#fff;padding:10px;display:none';
	uiDiv.appendChild(el1);
	...
	fs.appendChild(el1);
	el1 = wDoc.createElement('div');
	el1.style.cssText = 'height:0px;border:0px solid #808080;overflow:auto;';
...
/**/	el1 = wDoc.createElement('input');
	...
	el1.style.cssText = 'float:left;width:100px;margin-top:5px;display:none;';
	el1.setAttribute('id', 'tuBtnSelect');
	uiDiv.appendChild(el1);
	el1 = wDoc.createElement('input');
	el1.setAttribute('type', 'button');
	el1.setAttribute('value', 'Cancel');
	el1.style.cssText = 'float:right;width:100px;margin-top:5px;display:none;';
	...
	win.tuFileUploadStarted = function(pth, nme){
		function rntUrl(pth){
			return function(){
				...
				if( !(field_name=="onmouseoversrc" ||  field_name=="onmouseoutsrc"  )  )
					win.document.forms[0].elements[field_name].onchange();
	...
	{ 
	 if(xhr.readyState  == 4)
	 {
		if(xhr.status  == 200){ 
			...
			for (var i=0; i<imgList.length; i++){  //alert('AA'); alert(imgList[i].url);
