...
function off_the_charts($cat_id=-1,$type="",$user_id)
{
	...
	elseif($type=="2"){
		$active2="style='text-decoration:underline'";$active1="";$active3="";}
	else{		$active3="style='text-decoration:underline'"; $active1="";$active2="";}
	if($type==1) // My Monet
	{
...
function calculate_avg_valence($cf_ids)
{
	...
	while($data_c=mfa($rs))
	{
		...
		if(count($ary)>0)
		{
			...
			while($data=mfa($rs1))
			{
				if($i==1)$sec=(float)convert_to_sec($data[ad_time]);
...
function get_new_option_list_dis($table,$field,$id_col,$val_col,$sel_id,&$option,$show_all=0,$con="",$show_select=1)
{
    ...
    while($data = mfa($rs))
    {
	if($data[$id_col]==$sel_id){$sel="selected";$ss=0;}
        $option.="<option value=\"$data[$id_col]\" $sel>$data[$val_col]</option>";
        $sel="";
    }
    $sel="";
...
function get_nb_text($tot_rows,$title,$st_pos=0,&$con_limit,&$nb_text,$nrpp=0,$nspp=10)
{
    ...
    for($i=0;$i!=1;)
    {
	if(($k*$nrpp*$nspp)>$st_pos)	{	    $set=$k;	    $i=1;	}
	$k++;
    }
    $next=$set*$nrpp*$nspp;
    $back=$next-($nrpp*$nspp);
	...
	$pos=$i*$nrpp;
	$b1="";$b2="";
	if($pos==$st_pos){$b1="<b>";$b2="</b>";}
	$j=$i+1;
	$HL1="";$HL2=""; $color=$link_color;
	if($pos>=$tot_rows)	{	 continue;   $HL1="<!--";$HL2="-->"; $color=$unlink_color;	    }
	$con_mid.=<<<EOM
...
;
    ...
    $HFP1="";$HFP2="";	$HNL1="";$HNL2=""; $color_first=$link_color; $color_last=$link_color;
    if(($next-($nrpp*$nspp))==0)    		{	$HFP1="<!--";$HFP2="-->";    $color_first=$unlink_color;}
    if($next>=$tot_rows)    	{	$HNL1="<!--";$HNL2="-->";    $color_last=$unlink_color;}
    if($last>$tot_rows){$last=$last-$nrpp;}
...
function get_file_size($up_id,&$up_size)
{//$up_size = formatted upload size (including MB or KB etc)
    global $Upload_Path;
...
function resize_image($up_id,&$img_w,&$img_h,$img_dw=100)
{
    ...
    if(!file_exists($img_thumb1)){return;}
